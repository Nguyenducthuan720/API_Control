using Newtonsoft.Json;

namespace OsControl.Models.ShareData.PMS
{
    public static class CategoryGeneralSAP
    {
        public class Requets
        {
            public class Content
            {
                /// <summary>
                /// Code
                /// </summary>
                /// <example>AAA</example>
                public string? Code { get; set; }

                /// <summary>
                /// SAPID
                /// </summary>
                /// <example>SAPID</example>
                public string? SAPID { get; set; }

                /// <summary>
                /// CategoryType
                /// </summary>
                /// <example>AAA</example>
                public string? CategoryType { get; set; }

                /// <summary>
                /// Name
                /// </summary>
                /// <example>Test</example>
                public string? Name { get; set; }

                /// <summary>
                /// Name
                /// </summary>
                /// <example>Test</example>
                public string? NameExtention1 { get; set; }

                /// <summary>
                /// Name
                /// </summary>
                /// <example>Test</example>
                public string? NameExtention2 { get; set; }

                /// <summary>
                /// Name
                /// </summary>
                /// <example>Test</example>
                public string? NameExtention3 { get; set; }

                /// <summary>
                /// Name
                /// </summary>
                /// <example>Test</example>
                public string? NameExtention4 { get; set; }

                /// <summary>
                /// Name
                /// </summary>
                /// <example>Test</example>
                public string? NameExtention5 { get; set; }

                /// <summary>
                /// Name
                /// </summary>
                /// <example>Test</example>
                public string? NameExtention6 { get; set; }

                /// <summary>
                /// Name
                /// </summary>
                /// <example>Test</example>
                public string? NameExtention7 { get; set; }

                /// <summary>
                /// Name
                /// </summary>
                /// <example>Test</example>
                public string? NameExtention8 { get; set; }

                /// <summary>
                /// Name
                /// </summary>
                /// <example>Test</example>
                public string? NameExtention9 { get; set; }

                /// <summary>
                /// Phone
                /// </summary>
                /// <example>03214574</example>
                public string? Phone { get; set; }

                /// <summary>
                /// Address
                /// </summary>
                /// <example>Địa chỉ test 123</example>
                public string? Address { get; set; }

                /// <summary>
                /// Address
                /// </summary>
                /// <example>Địa chỉ test 123</example>
                public string? AddressExtention1 { get; set; }

                /// <summary>
                /// Address
                /// </summary>
                /// <example>Địa chỉ test 123</example>
                public string? AddressExtention2 { get; set; }

                /// <summary>
                /// Address
                /// </summary>
                /// <example>Địa chỉ test 123</example>
                public string? AddressExtention3 { get; set; }

                /// <summary>
                /// Address
                /// </summary>
                /// <example>Địa chỉ test 123</example>
                public string? AddressExtention4 { get; set; }

                /// <summary>
                /// Address
                /// </summary>
                /// <example>Địa chỉ test 123</example>
                public string? AddressExtention5 { get; set; }

                /// <summary>
                /// Address
                /// </summary>
                /// <example>Địa chỉ test 123</example>
                public string? AddressExtention6 { get; set; }

                /// <summary>
                /// Address
                /// </summary>
                /// <example>Địa chỉ test 123</example>
                public string? AddressExtention7 { get; set; }

                /// <summary>
                /// Address
                /// </summary>
                /// <example>Địa chỉ test 123</example>
                public string? AddressExtention8 { get; set; }

                /// <summary>
                /// Address
                /// </summary>
                /// <example>Địa chỉ test 123</example>
                public string? AddressExtention9 { get; set; }

                /// <summary>
                /// Dùng/không
                /// </summary>
                /// <example>1</example>
                public int? IsActive { get; set; }

                /// <summary>
                /// Extention
                /// </summary>
                /// <example>Mở rộng</example>
                public string? Extention1 { get; set; }

                /// <summary>
                /// Extention
                /// </summary>
                /// <example>Mở rộng</example>
                public string? Extention2 { get; set; }

                /// <summary>
                /// Extention
                /// </summary>
                /// <example>Mở rộng</example>
                public string? Extention3 { get; set; }

                /// <summary>
                /// Extention
                /// </summary>
                /// <example>Mở rộng</example>
                public string? Extention4 { get; set; }

                /// <summary>
                /// Extention
                /// </summary>
                /// <example>Mở rộng</example>
                public string? Extention5 { get; set; }

                /// <summary>
                /// Extention
                /// </summary>
                /// <example>Mở rộng</example>
                public string? Extention6 { get; set; }

                /// <summary>
                /// Extention
                /// </summary>
                /// <example>Mở rộng</example>
                public string? Extention7 { get; set; }

                /// <summary>
                /// Extention
                /// </summary>
                /// <example>Mở rộng</example>
                public string? Extention8 { get; set; }

                /// <summary>
                /// Extention
                /// </summary>
                /// <example>Mở rộng</example>
                public string? Extention9 { get; set; }
                public string? Extention10 { get; set; }
                public string? Extention11 { get; set; }
                public string? Extention12 { get; set; }
                public string? Extention13 { get; set; }
                public string? Extention14 { get; set; }
                public string? Extention15 { get; set; }
                public string? Extention16 { get; set; }
                public string? Extention17 { get; set; }
                public string? Extention18 { get; set; }
                public string? Extention19 { get; set; }
                public string? Extention20 { get; set; }

                /// <summary>
                /// InfoDescription
                /// </summary>
                /// <example>Mô tả ngắn</example>
                public string? InfoDescription { get; set; }
                public string? InfoDescriptionExtention1 { get; set; }
                public string? InfoDescriptionExtention2 { get; set; }
                public string? InfoDescriptionExtention3 { get; set; }
                public string? InfoDescriptionExtention4 { get; set; }
                public string? InfoDescriptionExtention5 { get; set; }
                public string? InfoDescriptionExtention6 { get; set; }
                public string? InfoDescriptionExtention7 { get; set; }
                public string? InfoDescriptionExtention8 { get; set; }
                public string? InfoDescriptionExtention9 { get; set; }

                /// <summary>
                /// InfoDetail
                /// </summary>
                /// <example>Mô tả chi tiết</example>
                public string? InfoDetail { get; set; }
                public string? InfoDetailExtention1 { get; set; }
                public string? InfoDetailExtention2 { get; set; }
                public string? InfoDetailExtention3 { get; set; }
                public string? InfoDetailExtention4 { get; set; }
                public string? InfoDetailExtention5 { get; set; }
                public string? InfoDetailExtention6 { get; set; }
                public string? InfoDetailExtention7 { get; set; }
                public string? InfoDetailExtention8 { get; set; }
                public string? InfoDetailExtention9 { get; set; }

                /// <summary>
                /// Name
                /// </summary>
                /// <example>Test</example>
                public string? Note { get; set; }
            }

            public class Details
            {
                /// <summary>
                /// Số thứ tự
                /// </summary>
                /// <example>1</example>
                public int? STT { get; set; }

                /// <summary>
                /// Mã code
                /// </summary>
                public string? Code { get; set; }

                /// <summary>
                /// Mã Lemon
                /// </summary>
                public string? LemonID { get; set; }

                /// <summary>
                /// Nhóm/Loại
                /// </summary>
                public string? Category { get; set; }

                /// <summary>
                /// Người dùng liên quan
                /// </summary>
                public int? UserID { get; set; }

                /// <summary>
                /// Tên tiếng Việt
                /// </summary>
                public string? Name { get; set; }

                /// <summary>
                /// Tên tiếng Anh
                /// </summary>
                public string? NameExtetion1 { get; set; }

                /// <summary>
                /// Ghi chú
                /// </summary>
                public string? Note { get; set; }

                /// <summary>
                /// Link đính kèm
                /// </summary>
                public string? Link { get; set; }

                /// <summary>
                /// Biển số xe
                /// </summary>
                public string? LicensePlates { get; set; }

                /// <summary>
                /// Năm sản xuất
                /// </summary>
                public string? ProductionYear { get; set; }

                /// <summary>
                /// Hạn sử dụng
                /// </summary>
                public string? ExpiryDate { get; set; }

                /// <summary>
                /// Màu xe (ID)
                /// </summary>
                public int? VehicleColorID { get; set; }

                /// <summary>
                /// Nhóm xe (ID)
                /// </summary>
                public int? VehicleGroupsID { get; set; }

                /// <summary>
                /// Thiết bị xe
                /// </summary>
                public string? VehicleDevices { get; set; }

                /// <summary>
                /// Mã loại chi phí
                /// </summary>
                public int? FeeID { get; set; }

                /// <summary>
                /// Chi tiết phí (dạng JSON hoặc danh sách con)
                /// </summary>
                public List<DetailFee> DetailFee { get; set; }

                /// <summary>
                /// Ghi chú chi phí
                /// </summary>
                public string? ExpenseNote { get; set; }
            }
            public class Expense
            {
                /// <summary>
                /// Mã chi phí
                /// </summary>
                /// <example>1</example>
                public int? FeeID { get; set; }

                /// <summary>
                /// Chi tiết chi phí
                /// </summary>
                public List<DetailFee> DetailFee { get; set; }

                /// <summary>
                /// Ghi chú
                /// </summary>
                /// <example>Ghi chú chi phí</example>
                public string? ExpenseNote { get; set; }
            }
            public class DetailFee
            {
                /// <summary>
                /// Mã thông tin tính toán
                /// </summary>
                /// <example>10</example>
                public int? TTID { get; set; }

                /// <summary>
                /// Giá trị
                /// </summary>
                /// <example>1000</example>
                public decimal Value { get; set; } = 0;
            }
            public class CategoryGenerals : Content
            {
                public string? CmpnID { get; set; }
                /// <summary>
                /// GeoCode
                /// </summary>
                /// <example>A|B</example>
                public string? GeoCode { get; set; }

                /// <summary>
                /// Address
                /// </summary>
                /// <example>Địa chỉ test 123</example>
                public string? Link { get; set; }

                /// <summary>
                /// LinkImg
                /// </summary>
                /// <example>LinkImg</example>
                public string? LinkImg { get; set; }

                /// <summary>
                /// Tham số
                /// </summary>
                /// <example></example>
                public decimal Parameter { get; set; }

                /// <summary>
                /// Tham số
                /// </summary>
                /// <example></example>
                public decimal ParameterExtention { get; set; }

                /// <summary>
                /// Trạng thái màu sắc
                /// </summary>
                /// <example>orange</example>
                public string? StatusColor { get; set; }

                /// <summary>
                /// Status
                /// </summary>
                /// <example></example>
                public int? Status { get; set; }

                /// <summary>
                /// IPAddress
                /// </summary>
                /// <example></example>
                public string? IPAddress { get; set; }

                /// <summary>
                /// FromHeight
                /// </summary>
                /// <example></example>
                public int? FromHeight { get; set; }

                /// <summary>
                /// ToHeight
                /// </summary>
                /// <example></example>
                public int? ToHeight { get; set; }

                /// <summary>
                /// FromWidth
                /// </summary>
                /// <example></example>
                public int? FromWidth { get; set; }

                /// <summary>
                /// ToWidth
                /// </summary>
                /// <example></example>
                public int? ToWidth { get; set; }

                /// <summary>
                /// PortNo
                /// </summary>
                /// <example></example>
                public string? PortNo { get; set; }

                /// <summary>
                /// ExcelType
                /// </summary>
                /// <example></example>
                public string? ExcelType { get; set; }

                /// <summary>
                /// FlashID
                /// </summary>
                /// <example></example>
                public int? FlashID { get; set; }

                /// <summary>
                /// ColorID
                /// </summary>
                /// <example></example>
                public int? ColorID { get; set; }

                /// <summary>
                /// IsShowApp
                /// </summary>
                /// <example></example>
                public int? IsShowApp { get; set; }

                /// <summary>
                /// Sort
                /// </summary>
                /// <example></example>
                public int? Sort { get; set; }

                /// <summary>
                /// LemonID
                /// </summary>
                /// <example></example>
                public string? LemonID { get; set; }

                /// <summary>
                /// Icon
                /// </summary>
                /// <example></example>
                public string? Icon { get; set; }

                /// <summary>
                /// Có phải là trạm
                /// </summary>
                /// <example>1</example>
                public int? IsStation { get; set; }

                /// <summary>
                /// Polygon khu vực trạm tàu hỏa trên bản đồ.
                /// </summary>
                /// <example>
                /// POLYGON((11.587238 108.986112,11.586878 108.987168,11.585263
                /// 108.986558,11.585665 108.985494,11.587238 108.986112))
                /// </example>
                public string? StationArea { get; set; }

                /// <summary>
                /// Vĩ độ
                /// </summary>
                /// <example>10.8</example>
                public decimal Lat { get; set; }

                /// <summary>
                /// Vĩ độ
                /// </summary>
                /// <example>106</example>
                public decimal Long { get; set; }

                /// <summary>
                /// Mã tuyến
                /// </summary>
                /// <example>1</example>
                public int? RouteID { get; set; }

                /// <summary>
                /// Mã tàu hỏa
                /// </summary>
                /// <example>SE07</example>
                public string? StationID { get; set; }

                /// <summary>
                /// NationID
                /// </summary>
                /// <example>1</example>
                public int? NationID { get; set; }

                /// <summary>
                /// ProvinceID
                /// </summary>
                /// <example>1</example>
                public int? ProvinceID { get; set; }

                /// <summary>
                /// DistrictID
                /// </summary>
                /// <example>1</example>
                public int? DistrictID { get; set; }

                /// <summary>
                /// Ward
                /// </summary>
                /// <example>1</example>
                public int? Ward { get; set; }
                public int? FromProvinceID { get; set; }
                public int? ToProvinceID { get; set; }
                public int? FromDistrictID { get; set; }
                public int? ToDistrictID { get; set; }
                public int? FromWardID { get; set; }
                public int? ToWardID { get; set; }
                public string? UserID { get; set; }
                public int? Distance { get; set; }

                /// <summary>
                /// UsageType
                /// </summary>
                /// <example>1</example>
                public string? UsageType { get; set; }

                /// <summary>
                /// Details
                /// </summary>
                public List<Details> Details { get; set; }

                /// <summary>
                /// Quy cách
                /// </summary>
                public List<Specification> Specifications { get; set; }

                /// <summary>
                /// Danh sách chi phí áp dụng
                /// </summary>
                public List<Expense> ExpenseDetails { get; set; }

                /// <summary>
                /// ID liên kết
                /// </summary>
                /// <example>1</example>
                public string? CategoryGeneralsID { get; set; }
            }
            public class Specification
            {
                /// <summary>
                /// ID
                /// </summary>
                /// <example></example>
                public int? ID { get; set; }

                /// <summary>
                /// Quy cách 1
                /// </summary>
                /// <example>0</example>
                public string? Extention1 { get; set; }

                /// <summary>
                /// Quy cách 2
                /// </summary>
                /// <example>0</example>
                public string? Extention2 { get; set; }

                /// <summary>
                /// Quy cách 3
                /// </summary>
                /// <example>0</example>
                public string? Extention3 { get; set; }

                /// <summary>
                /// Quy cách 4
                /// </summary>
                /// <example>0</example>
                public string? Extention4 { get; set; }

                /// <summary>
                /// Quy cách 5
                /// </summary>
                /// <example>0</example>
                public string? Extention5 { get; set; }

                /// <summary>
                /// Quy cách 6
                /// </summary>
                /// <example>0</example>
                public string? Extention6 { get; set; }

                /// <summary>
                /// Quy cách 7
                /// </summary>
                /// <example>0</example>
                public string? Extention7 { get; set; }

                /// <summary>
                /// Quy cách 8
                /// </summary>
                /// <example>0</example>
                public string? Extention8 { get; set; }

                /// <summary>
                /// Quy cách 9
                /// </summary>
                /// <example>0</example>
                public string? Extention9 { get; set; }

                /// <summary>
                /// Quy cách 10
                /// </summary>
                /// <example>0</example>
                public string? Extention10 { get; set; }
            }
            public class Get
            {
                public string? CategoryType { get; set; }
            }
            public class GetByID
            {
                public string? SAPID { get; set; }
                public string? CategoryType { get; set; }
            }

            public class Add : CategoryGenerals
            {
            }

            public class Edit : CategoryGenerals
            {
                /// <summary>
                /// ID
                /// </summary>
                /// <example>1</example>
                public string? SAPID { get; set; }
            }

        }
    }
}