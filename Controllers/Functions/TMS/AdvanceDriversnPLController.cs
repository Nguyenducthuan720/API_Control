...
using static APISmartCity.Models.TMS.AdvanceDrivers.Request;

namespace APISmartCity.Controllers.TMS
{
    [ApiExplorerSettings(GroupName = "Microservice.nPL.AdvanceDrivers")]
    [Route("api/[controller]/[action]")]
    [Authorize]
    [ApiController]
    public class AdvanceDriversController : ControllerBase
    {
        private readonly UserInfo _UserInfo;
        private readonly string _ConfigurationDB;
        private readonly string _nPLDB;
        private readonly string _ProcedureName = "ExecAdvanceDrivers";

        public AdvanceDriversController(UserInfo userInfo)
        {
        ...
        public async Task<IActionResult> Add([FromBody] Add request)
        {
            try
            {
                ...
                if (request.AdvanceDriversDetails != null)
                {
                    DataTable AdvanceDriversDetail = request.AdvanceDriversDetails.ToDataTable();
                    AdvanceDriversDetail.SetTypeName("AdvanceDriversDetails");
                    parameters.Add("@AdvanceDriversDetails", AdvanceDriversDetail);
                    request.AdvanceDriversDetails = null;
                }
                DataResponse dataResponse = await GetDataResponse(parameters, _nPLDB, _ConfigurationDB, _ProcedureName, request);
                return Ok(dataResponse);
            }
