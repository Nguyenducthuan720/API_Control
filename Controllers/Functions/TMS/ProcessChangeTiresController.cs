...
using static APISmartCity.Models.TMS.ProcessChangeTires.Request;

namespace APISmartCity.Controllers.TMS
{
    [ApiExplorerSettings(GroupName = "Microservice.nPL.ProcessChangeTires")]
    [Route("api/[controller]/[action]")]
    [Authorize]
    [ApiController]
    public class ProcessChangeTiresController : ControllerBase
    {
        private readonly UserInfo _UserInfo;
        private readonly string _ConfigurationDB;
        private readonly string _nPLDB;
        private readonly string _ProcedureName = "ExecProcessChangeTires";

        public ProcessChangeTiresController(UserInfo userInfo)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> GetTire([FromBody] GetTire request)
        {
            try
            {
                Dictionary<string, object> parameters = new()
        ...
        public async Task<IActionResult> GetById([FromBody] GetByID request)
        {
            try
            {
                ...
                if (dataResponse.Result!.Count > 1)
                {
                    ...
                    result.Tires = dataResponse.Result[1];
        ...
        public async Task<IActionResult> Add([FromBody] Add request)
        {
            try
            {
                ...
                if (request.Tires != null)
                {
                    DataTable Tires = request.Tires.ToDataTable();
                    Tires.SetTypeName("Tires");
                    parameters.Add("@Tires", Tires);
                    request.Tires = null;
                }
                DataResponse dataResponse = await GetDataResponse(parameters, _nPLDB, _ConfigurationDB, _ProcedureName, request);
                return Ok(dataResponse);
