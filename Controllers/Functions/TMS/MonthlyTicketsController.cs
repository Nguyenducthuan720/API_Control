...
using static APISmartCity.Models.TMS.MonthlyTickets.Request;

namespace APISmartCity.Controllers.TMS
{
    [ApiExplorerSettings(GroupName = "Microservice.nPL.MonthlyTickets")]
    ...
    public class MonthlyTicketsController : ControllerBase
    {
        ...
        private readonly string _ProcedureName = "ExecMonthlyTickets";

        public MonthlyTicketsController(UserInfo userInfo)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> GetInfo([FromBody] GetInfo request)
        {
            try
            {
                Dictionary<string, object> parameters = new()
        ...
        public async Task<IActionResult> Add([FromBody] Add request)
        {
            try
            {
                ...
                if (request.MonthlyTicketsDetails != null)
                {
                    DataTable MonthlyTicketsDetail = request.MonthlyTicketsDetails.ToDataTable();
                    MonthlyTicketsDetail.SetTypeName("MonthlyTicketsDetails");
                    parameters.Add("MonthlyTicketsDetails", MonthlyTicketsDetail);
                    request.MonthlyTicketsDetails = null;
                }
                DataResponse dataResponse = await GetDataResponse(parameters, _nPLDB, _ConfigurationDB, _ProcedureName, request);
                return Ok(dataResponse);
            }
