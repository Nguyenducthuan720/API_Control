...
{
    [ApiExplorerSettings(GroupName = "Microservice.Category.DataTypes")]
    ...
    public class DataTypesController : ControllerBase
    {
        ...
        private readonly string _ProcedureName = "ExecDataTypes";

        public DataTypesController(UserInfo userInfo)
        {
            _UserInfo = userInfo;
            _ConfigurationDB = Global.ListDB?.Find(item => item.DBType == "CON")?.DBString!;
            _CategoryDB = Global.ListDB?.Find(item => item.DBType == "CTL")?.DBString!;
            _SettingOther = Global.ListFolder?.Find(item => item.Type == "DataTypes");
        }

        /// <summary>
        /// Load list DataTypes
        /// </summary>
        /// <remarks>Load list DataTypes</remarks>
        [HttpPost]
        ...
        /// <summary>
        /// Lấy thông tin DataType theo ID
        /// </summary>
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> Add([FromBody] DataTypes.Request.Add request)
        {
            try
            {
                Dictionary<string, object> parameters = new()
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> Edit([FromBody] DataTypes.Request.Edit request)
        {
            try
            {
                Dictionary<string, object> parameters = new()
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> Delete([FromBody] DataTypes.Request.Del request)
        {
            try
            {
                Dictionary<string, object> parameters = new()
        ...
        /// <summary>
        /// Lấy thông tin Monitor theo Menu
        /// </summary>
        /// <param name="request"></param>
        /// <returns></returns>
        [HttpPost]
        public async Task<IActionResult> GetDataTypeByScreen([FromBody] DataTypes.Request.ByScreen request)
        {
            try
            {
                Dictionary<string, object> parameters = new()
        ...
        [HttpPost]
        [ProducesResponseType(200, Type = typeof(DataResponse))]
        public async Task<IActionResult> GetRight()
        {
            try
            {
                Dictionary<string, object> parameters = new()
